...
async function exchangeCodeForTokens(code) {
  ...
  try {
    ...
    throw new Error(
      `Token exchange failed: Invalid JSON response from Spotify`
    );
  }
...
async function refreshSpotifyToken(tokens) {
  ...
  try {
    ...
    throw new Error(`Token refresh failed: Invalid JSON response from Spotify`);
  }
...
// Improved Spotify API call with better error handling
async function makeSpotifyApiCall(url, options = {}) {
  console.log(`🌐 Making Spotify API call to: ${url}`);
  ...
  try {
    ...
    if (responseText.includes("<html") || responseText.includes("<!DOCTYPE")) {
      throw new Error(
        `Spotify API returned HTML instead of JSON. This usually indicates an authentication or configuration issue. Response preview: ${responseText.substring(
          0,
          100
        )}`
      );
    ...
    throw new Error(
      `Spotify API returned invalid JSON. Response: ${responseText.substring(
        0,
        200
      )}`
    );
...
1. Your Spotify app credentials are correct
2. Your app has the necessary scopes enabled
3. The redirect URI matches exactly what's configured in your Spotify app`,
        },
      ],
      isError: true,
    };
...
function createSpotifyMcpServer(sessionId) {
  ...
  server.registerTool(
    ...
    async () => {
      return await handleSpotifyTool(sessionId, async (accessToken) => {
        const data = await makeSpotifyApiCall("https://api.spotify.com/v1/me", {
          headers: {
            Authorization: `Bearer ${accessToken}`,
          },
        });
  ...
  server.registerTool(
    ...
    async ({ name, description = "", public: isPublic }) => {
      return await handleSpotifyTool(sessionId, async (accessToken) => {
        // Get user ID first
        const userData = await makeSpotifyApiCall(
...
app.get("/health", (req, res) => {
  res.json({
    ...
    timestamp: new Date().toISOString(),
    activeSessions: transports.size,
    activeTokens: sessionTokens.size,
  });
});
      ...
      <body>
        <div class="container">
          ...
          <div class="debug-info">
            ...
            Client ID: ${SPOTIFY_CLIENT_ID}<br>
      ...
      <html>
        ...
        <body>
          <div class="container">
            ...
            <p>Please try the authentication process again or check your Spotify app configuration.</p>
          </div>
        </body>
      </html>
    `);
